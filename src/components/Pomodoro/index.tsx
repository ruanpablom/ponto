...
export function Pomodoro(): JSX.Element {
    ...
    setIsBreak(prev => {
      if (prev) {
        ...
        setBreakCount(prevCount =>
          prevCount >= POMODORO_BREAK_TIMES.length - 1 ? 0 : prevCount + 1,
        );
      }
  ...
  const minutes = String(
    Math.floor((pomodoroTimer % (1000 * 60 * 60)) / (1000 * 60)),
  ).padStart(2, '0');
  const seconds = String(
    Math.floor((pomodoroTimer % (1000 * 60)) / 1000),
  ).padStart(2, '0');
  ...
  return (
    <div className="flex flex-col bg-white/10 p-10 rounded-xl gap-2 w-80">
      ...
      {!isCounting ? (
        <button
          className={`shadowButton bg-white font-bold py-2 rounded-md ${
